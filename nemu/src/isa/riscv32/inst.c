...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  switch (type) {
    ...
    case TYPE_Is:src1R();      *imm = rs2; break;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //TODO: use 'signed_t' replace 'int32_t'
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = (int32_t)(uint8_t)Mr(src1+imm, 1));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, if(src2 == 0){R(rd) = (int32_t)-1;} else{R(rd) = (int32_t)src1 / (int32_t)src2;});
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, if(src2 == 0){R(rd) = (int32_t)-1;} else{R(rd) = src1 / src2;});
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, if(src2 == 0){R(rd) = src1;} else{R(rd) = (int32_t)src1 % (int32_t)src2;});
